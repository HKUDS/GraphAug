...
class GraphLearner(nn.Module):
    def __init__(self, input_size, hidden_size, graph_type, top_k=None, epsilon=None, num_pers=4, metric_type="attention",
        ...
        self.decoder_edge = nn.Sequential(nn.LeakyReLU(), nn.Linear(self.hidden_size, self.hidden_size), nn.LeakyReLU(), nn.Linear(self.hidden_size, 1)).cuda()
        ...
        if metric_type == 'attention':
            ...
            self.lin1 = nn.Linear(self.input_size, self.hidden_size).cuda()
            self.lin2 = nn.Linear(self.hidden_size, self.hidden_size).cuda()
    ...
    def reset_parameters(self):
        ...
        if self.metric_type == 'attention':
            ...
            self.linear_sim1.reset_parameters().cuda()
            self.linear_sim2.reset_parameters().cdua()
    ...
    def forward(self, node_features, new_edge_index):
        if self.feature_denoise:
            masked_features = self.mask_feature(node_features)
            learned_adj = self.learn_adj(masked_features, new_edge_index)
            return masked_features, learned_adj
        else:
            learned_adj = self.learn_adj(node_features, new_edge_index)
            return node_features, learned_adj

    def learn_adj(self, context, edge_index, ctx_mask=None):
        """
        Parameters
        ...
        if self.metric_type == 'attention':
            ...
            context_fc = context.unsqueeze(0) * expand_weight_tensor.cuda()
            context_norm = F.normalize(context_fc, p=2, dim=-1)
            attention = torch.matmul(context_norm, context_norm.transpose(-1, -2)).mean(0)
            markoff_value = 0

        elif self.metric_type == 'transformer':
            Q = self.linear_sim1(context).cuda()
            attention = torch.matmul(Q[edge_index[0]], Q[edge_index[1]].transpose(-1, -2)) / math.sqrt(Q.shape[-1])
            markoff_value = -INF
            ...
            for _ in range(len(self.linear_sims1)):
                ...
                a_input1 = self.linear_sims1[_].cuda()(context)
               
                a_input2 = self.linear_sims2[_].cuda()(context)
                
                eleWise_mul = a_input1[edge_index[0]] + a_input2[edge_index[1]]
                
                attention.append(self.leakyrelu(eleWise_mul))

            attention = torch.mean(torch.stack(attention, 0), 0).squeeze().cuda()
            
            markoff_value = -INF
            # markoff_value = 0

        elif self.metric_type == 'kernel':
            # dist_weight = torch.mm(self.weight[edge_index[0]], self.weight[edge_index[1]].transpose(-1, -2)).cuda()
            print(self.weight[edge_index[0]].size(), self.weight[edge_index[1]].size())
            dist_weight = torch.mm(self.weight[edge_index[0]], self.weight[edge_index[1]]).cuda()
            attention = self.compute_distance_mat(context, dist_weight).cuda()
            attention = torch.exp(-0.5 * attention * (self.precision_inv_dis**2)).cuda()

            markoff_value = 0

        elif self.metric_type == 'cosine':
            context_norm = context.div(torch.norm(context, p=2, dim=-1, keepdim=True))
            attention = torch.mm(context_norm[edge_index[0]], context_norm[edge_index[1]].transpose(-1, -2)).detach()
            markoff_value = 0
        elif self.metric_type == 'mlp':
            context_fc = torch.relu(self.lin2(torch.relu(self.lin1(context))))
            
            eleWise_mul = context_fc[edge_index[0]] * context_fc[edge_index[1]]
           
            prob = nn.LogSoftmax(dim=0)(self.decoder_edge(eleWise_mul)).squeeze()
            prob = torch.exp(prob)
            
            
            prob[prob < self.epsilon] = 0
            prob[prob >= self.epsilon] = 1
            
            idxs = edge_index
            vals = prob
            shape = torch.Size((context_fc.size(0), context_fc.size(0)))
    ...
    def build_knn_neighbourhood(self, attention, top_k, markoff_value):
        # print("build attention:", attention.size())
        ...
        # top_k = min(top_k, edge_weight.size(-1))
        # knn_val, knn_ind = torch.topk(edge_weight, top_k, dim=-1)
        # weighted_adjacency_matrix1 = (markoff_value * torch.ones_like(edge_weight)).scatter_(-1, knn_ind, knn_val)
        # weighted_adjacency_matrix1 = weighted_adjacency_matrix1.to(device)
        # weighted_adjacency_matrix = (weighted_adjacency_matrix + weighted_adjacency_matrix1)/2
